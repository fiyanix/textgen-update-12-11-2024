...
import { reverseText } from '../../utils/textTransformers';
...
export default function ReverseText() {
  ...
  const getFormattedText = (format: 'text' | 'html' | 'svg' | 'markdown') => {
    ...
    switch (format) {
      case 'html':
        return `<div style="font-family: system-ui; background: #111827; color: #34D399; padding: 1rem; border-radius: 0.5rem;">${reversed}</div>`;
      case 'markdown':
        return '> ' + reversed + ' ←';
      case 'svg':
        return `<svg width="${reversed.length * 14}" height="40" xmlns="http://www.w3.org/2000/svg">
          <style>text { font-family: system-ui; font-size: 24px; }</style>
          <text x="10" y="30" fill="#34D399">${reversed}</text>
        </svg>`;
      default:
        return reversed;
    }
  ...
  const handleDownload = (format: 'txt' | 'html' | 'svg') => {
    try {
      ...
      a.href = url;
      a.download = `reverse-text-${Date.now()}.${format}`;
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          ...
          <p className="text-gray-600 max-w-2xl mx-auto">
            Transform your text by reversing its order. Create intriguing messages, 
            play with words, or add a unique twist to your content.
          </p>
        </div>
            ...
            <div className="border-t pt-6">
              ...
              <div className="relative bg-gray-900 rounded-lg p-4 min-h-[8rem] overflow-x-auto">
                <div className="text-green-400 font-mono whitespace-pre">
                  {input ? reverseText(input) : 'Your reversed text will appear here...'}
                </div>
        ...
        <div className="mt-12 bg-white rounded-xl shadow-lg p-6">
          ...
          <ul className="space-y-3 text-gray-600">
            ...
            <li className="flex items-start">
              ...
              <p>Create palindromes by checking if the reversed text matches the original</p>
            </li>
          </ul>
        </div>
      </div>
