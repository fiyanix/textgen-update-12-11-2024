...
import { FlipVertical, Sparkles, Share2, Palette } from 'lucide-react';
...
export default function InvertedText() {
  ...
  const getFormattedText = (format: 'text' | 'html' | 'svg' | 'markdown') => {
    const invertedText = invertText(input);
    switch (format) {
      case 'html':
        return `<div style="font-family: system-ui; background: #111827; color: #34D399; padding: 1rem; border-radius: 0.5rem;">${invertedText}</div>`;
      case 'markdown':
        return '> ' + invertedText;
      case 'svg':
        return `<svg width="${invertedText.length * 14}" height="40" xmlns="http://www.w3.org/2000/svg">
          <style>text { font-family: system-ui; font-size: 24px; }</style>
          <text x="10" y="30" fill="#34D399">${invertedText}</text>
        </svg>`;
      default:
        return invertedText;
    }
  ...
  const handleDownload = (format: 'txt' | 'html' | 'svg') => {
    try {
      ...
      a.href = url;
      a.download = `inverted-text-${Date.now()}.${format}`;
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          <div className="flex items-center justify-center mb-4">
            <div className="p-3 bg-indigo-100 rounded-full">
              <FlipVertical className="h-8 w-8 text-indigo-600" />
            </div>
          </div>
          ...
          <p className="text-gray-600 max-w-2xl mx-auto">
            Transform your text upside down with our advanced text inverter. Create eye-catching messages 
            for social media, design unique signatures, or add a creative twist to your digital content.
          </p>
        </div>
            ...
            <div className="border-t pt-6">
              ...
              <div className="relative bg-gray-900 rounded-lg p-4 min-h-[8rem] overflow-x-auto">
                <div className="text-green-400 font-mono whitespace-pre">
                  {input ? invertText(input) : 'Your inverted text will appear here...'}
                </div>
        ...
        <div className="mt-12 grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-white p-6 rounded-xl shadow-lg">
            <div className="flex items-center mb-4">
              ...
              <h3 className="font-semibold text-gray-900">Styled Export</h3>
            </div>
            <p className="text-gray-600">
        ...
        <div className="mt-12 bg-white rounded-xl shadow-lg p-6">
          <h2 className="text-xl font-semibold text-gray-900 mb-6">
            Popular Uses for Inverted Text
          </h2>
          <div className="space-y-4">
            <div className="p-4 bg-gray-50 rounded-lg">
              <h3 className="font-medium text-gray-900 mb-2">Social Media Bio</h3>
              <p className="text-gray-600">Make your profile stand out with a unique upside-down description</p>
              <div className="mt-2 p-3 bg-white rounded border border-gray-200">
                <p className="text-sm text-gray-500">Example: {invertText("✨ Creative Soul")}</p>
              </div>
            </div>
            <div className="p-4 bg-gray-50 rounded-lg">
              <h3 className="font-medium text-gray-900 mb-2">Message Emphasis</h3>
              <p className="text-gray-600">Add emphasis to important parts of your message</p>
              <div className="mt-2 p-3 bg-white rounded border border-gray-200">
                <p className="text-sm text-gray-500">Example: Don't forget to {invertText("SMILE")} today!</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mt-12 bg-white rounded-xl shadow-lg p-6">
          ...
          <ul className="space-y-3 text-gray-600">
            ...
            <li className="flex items-start">
              ...
              <p>Mix regular and inverted text for creative effects</p>
            </li>
          </ul>
        </div>
      </div>
