...
import { toCircleText } from '../../utils/textTransformers';
...
export default function CircleText() {
  ...
  const getFormattedText = (format: 'text' | 'html' | 'svg' | 'markdown') => {
    const circleText = toCircleText(input, style);
    switch (format) {
      case 'html':
        return `<div style="font-family: system-ui; background: #111827; color: #34D399; padding: 1rem; border-radius: 0.5rem;">${circleText}</div>`;
      case 'markdown':
        return '> ' + circleText + (style === 'outlined' ? ' ⭕' : ' 🔵');
      case 'svg':
        return `<svg width="${circleText.length * 14}" height="40" xmlns="http://www.w3.org/2000/svg">
          <style>text { font-family: system-ui; font-size: 24px; }</style>
          <text x="10" y="30" fill="#34D399">${circleText}</text>
        </svg>`;
      default:
        return circleText;
    }
  ...
  const handleDownload = (format: 'txt' | 'html' | 'svg') => {
    try {
      ...
      a.href = url;
      a.download = `circle-text-${Date.now()}.${format}`;
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          ...
          <p className="text-gray-600 max-w-2xl mx-auto">
            Transform your text into elegant circled letters. Choose between outlined or 
            filled circles for different styles and effects.
          </p>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-lg p-6">
          <div className="space-y-6">
            ...
            <TextInput
              ...
              onReset={() => setInput('')}
              placeholder="Type your text here to add circles..."
              label="Original Text"
            />
            ...
            <div className="border-t pt-6">
              ...
              <div className="relative bg-gray-900 rounded-lg p-4 min-h-[8rem] overflow-x-auto">
                <div className="text-green-400 font-mono whitespace-pre">
                  {input ? toCircleText(input, style) : 'Your circle text will appear here...'}
                </div>
        ...
        <div className="mt-12 bg-white rounded-xl shadow-lg p-6">
          ...
          <ul className="space-y-3 text-gray-600">
            ...
            <li className="flex items-start">
              ...
              <p>Switch between outlined and filled styles for different effects</p>
            </li>
          </ul>
        </div>
      </div>
