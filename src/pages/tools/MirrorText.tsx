...
import { mirrorText } from '../../utils/textTransformers';
...
export default function MirrorText() {
  ...
  const getFormattedText = (format: 'text' | 'html' | 'svg' | 'markdown') => {
    ...
    switch (format) {
      case 'html':
        return `<div style="font-family: system-ui; background: #111827; color: #34D399; padding: 1rem; border-radius: 0.5rem;">${mirrored}</div>`;
      case 'markdown':
        return '> ' + mirrored + ' ←';
      case 'svg':
        return `<svg width="${mirrored.length * 14}" height="40" xmlns="http://www.w3.org/2000/svg">
          <style>text { font-family: system-ui; font-size: 24px; }</style>
          <text x="10" y="30" fill="#34D399">${mirrored}</text>
        </svg>`;
      default:
        return mirrored;
    }
  ...
  const handleDownload = (format: 'txt' | 'html' | 'svg') => {
    try {
      ...
      a.href = url;
      a.download = `mirror-text-${Date.now()}.${format}`;
        ...
        <div className="text-center mb-12">
          <div className="flex items-center justify-center mb-4">
            <div className="p-3 bg-indigo-100 rounded-full">
              <FlipHorizontal className="h-8 w-8 text-indigo-600" />
            </div>
          </div>
          ...
          <p className="text-gray-600 max-w-2xl mx-auto">
            Create mirror-image text that reads backwards, perfect for artistic designs, 
            creative projects, and unique visual effects.
          </p>
        </div>
            ...
            <div className="border-t pt-6">
              ...
              <div className="relative bg-gray-900 rounded-lg p-4 min-h-[8rem] overflow-x-auto">
                <div className="text-green-400 font-mono whitespace-pre">
                  {input ? mirrorText(input) : 'Your mirrored text will appear here...'}
                </div>
        ...
        <div className="mt-12 bg-white rounded-xl shadow-lg p-6">
          ...
          <ul className="space-y-3 text-gray-600">
            ...
            <li className="flex items-start">
              ...
              <p>Mix regular and mirrored text for creative effects</p>
            </li>
          </ul>
        </div>
      </div>
